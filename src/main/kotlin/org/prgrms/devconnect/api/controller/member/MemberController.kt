...
class MemberController(
  ...
  fun getMember(@AuthenticationPrincipal member: Member): ResponseEntity<MemberResponseDto> {
    ...
    return ResponseEntity.ok<MemberResponseDto>(responseDto)
  }
  ...
  fun logout(@AuthenticationPrincipal member: Member): ResponseEntity<Void> {
    memberCommandService.logout(member.email)
    return ResponseEntity.ok().build()
  }

  @PutMapping
  @Operation(summary = "회원 정보 수정", description = "현재 회원의 정보를 수정합니다.")
  fun updateMember(@AuthenticationPrincipal member: Member,
                   @RequestBody dto: @Valid MemberUpdateRequestDto): ResponseEntity<Void> {
