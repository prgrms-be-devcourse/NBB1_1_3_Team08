...
class JobPostApi(
    ...
    fun fetchJobPosts(start: Int): List<JobPost> {
        ...
        try {
            ...
            val apiURL = "https://oapi.saramin.co.kr/job-search?access-key=$accessKey&job_cd=2232+84+92+291+277+213+235+236+302+272+87+195&job_mid_cd=2&start=$start&count=100"
            val response = xmlParser(apiURL)
            val xmlMapper = XmlMapper()
            val jobSearchResponse = xmlMapper.readValue(response.toString(), JobSearchResponse::class.java)
            ...
            jobSearchResponse.jobs?.jobList?.forEach { job ->
                ...
                if (!jobPostQueryService.isJobPostByPostId(postId)) {
                    ...
                    val techStacks = saveTechStackByJobCode(job.position?.jobCode) // JobCode에 따라 TechStack 리스트 생성
    ...
    fun getTotal(start: Int): Int {
        return try {
            val encodedKeyword = URLEncoder.encode("", StandardCharsets.UTF_8)
            val apiURL = "https://oapi.saramin.co.kr/job-search?access-key=$accessKey&job_cd=2232+84+92+291+277+213+235+236+302+272+87+195&job_mid_cd=2&start=$start&count=1"
            val response = xmlParser(apiURL)
            val xmlMapper = XmlMapper()
            val jobSearchResponse = xmlMapper.readValue(response.toString(), JobSearchResponse::class.java)
    ...
    private fun extractCompanyInfoFromHtml(companyPageLink: String?): CompanyHtmlResponse {
        ...
        try {
            if (companyPageLink != null) {
                ...
                address = companyPage.selectFirst("div.company_details_group dt.tit:contains(주소) + dd.desc p.ellipsis")?.attr("title")?.trim()
            }
        } catch (e: Exception) {
            println(e)
        }
    ...
    fun saveTechStackByJobCode(jobCode: String?): List<TechStack> {
        val techStackMap = mapOf(
                "웹개발" to TechStack("웹개발", "87"),
                "풀스택" to TechStack("풀스택", "2232"),
                "백엔드/서버개발" to TechStack("백엔드", "84"),
                "프론트엔드" to TechStack("프론트엔드", "92"),
                "Android" to TechStack("Android", "195"),
                "Spring" to TechStack("Spring", "291"),
                "JPA" to TechStack("JPA", "238"),
                "React" to TechStack("React", "277"),
                "Django" to TechStack("Django", "213"),
                "Java" to TechStack("Java", "235"),
                "Javascript" to TechStack("Javascript", "236"),
                "TypeScript" to TechStack("TypeScript", "302"),
                "Python" to TechStack("Python", "272"),
                "C++" to TechStack("C++", "205"),
                "Kotlin" to TechStack("Kotlin", "243"),
                "AWS" to TechStack("AWS", "201"),
                "Linux" to TechStack("Linux", "246")
        )
